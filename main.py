...
def chatPanel():
    ...
    maxScroll = max(0, len(lines) - visibleHieght)
    global chatScroll
    chatScroll = min(chatScroll, maxScroll)
    start = max(0, len(lines) - visibleHieght - chatScroll)
...
def cpuStats():
    ...
    for i, usage in enumerate(cpuCoresUsage):
        core = times[i]
        text += ("[white]" + "Core " + str(i + 1) + " Usage:  " + "[/white]" + "[red]" + str(fullbar(usage)) + "[/red]" + "[white]" + str(usage) + "%  " + "[/white]" + "[white]" + "[#8fb6d8]" + " U:" + "[/#8fb6d8]" + str(core.user) + "  " + "[#9fca8c]" + "S:" + "[/#9fca8c]" + str(core.system) + "  " + "[#b6a6c8]" + "I:" + "[/#b6a6c8]" + str(core.idle) + "  " + "[/white]" +"\n"
        )
...
def gpuStats():
    ...
    try:
        ...
        #GPU power draw — watts, feels real/pro-monitor-ish
        #GPU memory used
        #GPU frequency
        #temperature if available
        #5–10 sec usage history sparkline
        stats = macgputils.get_gpu_stats()
        if isinstance(stats, list):
            stats = stats[0]
        gpuPower = float(stats.get("GPU Power", 0.0).replace("mW", " ").strip())
        gpuAR = float(stats.get("HW Active Residency", 0.0).replace("%", " ").strip())
        gpuIR = float(stats.get("Idle Residency", 0.0).replace("%", " ").strip())
        gpuFreq = float(stats.get("Active Frequency", 0.0).replace("MHz", " ").strip())
        output = result.stdout
        match = re.search(r"GPU HW active residency:\s+([\d.]+)%", output)
        gpuTextLabel = len("[green]GPU USAGE ")
        gpuBar = len("")
        ...
        if match:
                ...
                gpuText = (
                    ...
                    f"[yellow]Power Draw: [/yellow]{spacerLength('Power Draw:', powerText)}[yellow] {gpuPower:.0f} Mv [/yellow] \n"
...
def memdeskStats():
    ...
    text += f"[bold white] DISK Total:[/bold white] {disk.total:.1f}\n"
    ...
    diskFreeText = f"Avalible: {diskFree:.0f}/{diskFree:.0f} GiB"
    diskFreeBar = f"{diskFreePercent:.0f}% " + dotBlock(diskFreePercent)

    text += f"[yellow]Avalible:[/yellow] {diskFree:.0f}/{diskFree:.0f} GiB {spacerLength(diskFreeText, diskFreeBar)}"
...
def basicProcesses():
    ...
    for process, cpu, mem in final_processes[:22]:
        ...
        coloredBar = (
            "[red]CPU: [/red]" + str(cpu) + "% " + "[#6f89a8]MEM: [/]" + str(round(mem, 3)) + "%"
        )
